...
def get_slot_machine_spin(rows, cols, symbols):
    ...
    columns = [[] for _ in range(cols)]  # Initialize empty columns
...
def print_slot_machine(columns):
    for row in range(len(columns[0])):  # Assuming all columns have the same number of rows
        for col in range(len(columns)):
            print(columns[col][row], end=" | " if col != len(columns) - 1 else "\n")
